...
$showProps = !empty($arResult['DISPLAY_PROPERTIES']) || $arResult['SHOW_OFFERS_PROPS'];
...
$APPLICATION->SetPageProperty("og:description",strip_tags(html_entity_decode($arResult["DETAIL_TEXT"])));
$APPLICATION->SetPageProperty("twitter:description",strip_tags(html_entity_decode($arResult["DETAIL_TEXT"])));
$APPLICATION->SetPageProperty("og:image",'http://kristallgold.ru/upload/'.$arResult["PREVIEW_PICTURE"]["SUBDIR"].'/'.$arResult["PREVIEW_PICTURE"]["FILE_NAME"]);
$APPLICATION->SetPageProperty("twitter:image",'http://kristallgold.ru/upload/'.$arResult["PREVIEW_PICTURE"]["SUBDIR"].'/'.$arResult["PREVIEW_PICTURE"]["FILE_NAME"]);
		...
		<div class="row">
			<div class="col-md-6 col-sm-12">
				<div class="product-item-detail-slider-container" id="<?=$itemIds['BIG_SLIDER_ID']?>">
							...
							<div class="product-item-label-text <?=$labelPositionClass?>" id="<?=$itemIds['STICKER_ID']?>"
								...
								if ($arResult['LABEL'] && !empty($arResult['LABEL_ARRAY_VALUE']))
								{
									...
									foreach ($arResult['LABEL_ARRAY_VALUE'] as $code => $value)
									{
										...
										switch ($code) {
											case 'NEWPRODUCT':
											$value = GetMessage("NEWPRODUCT");
											$class = " item_".strtolower($code);
											break;
											case 'BESTSELLER':
											$value = GetMessage("BESTSELLER");
											$class = " item_".strtolower($code);
											break;
											case 'DISCOUNT':
											$value = GetMessage("DISCOUNT");
											$class = " item_".strtolower($code);
											break;
											default: break;
										}
										?>
										<div<?=(!isset($arParams['LABEL_PROP_MOBILE'][$code]) ? ' class="hidden-xs'.$class.'"' : $class)?>>
					...
					?>
					<div class="product-item-detail-info-section" style="text-align: center; font-weight: bold; margin: 10px;">
						В некоторых случаях украшение может отличаться от представленного на фото.
					</div>
				</div>
			</div>
			<div class="col-md-6 col-sm-12">
				...
				<div class="artnumb_rating">
					<div class="detail-item-artnumber">
						Артикул: <?=$arResult['PROPERTIES'][$articul]['VALUE'];?>
					</div>
					<div class="rating">
				...
				<div class="product-item-detail-info-section">
					<?array_splice($arParams['PRODUCT_INFO_AND_PAYMENT_BLOCK_ORDER'], 1, 0, "har");?>
					<?
					foreach ($arParams['PRODUCT_INFO_AND_PAYMENT_BLOCK_ORDER'] as $blockName) {
						switch ($blockName)
						{
							case 'har':
							?>
					...
					<div id="<?=$itemIds['TREE_ID']?>">
						<?if ($haveOffers && !empty($arResult['OFFERS_PROP']))
						{
							...
							<?
							$dropdownSku = array();
							$tmp_dropdownSku = explode(",",COption::GetOptionString("uvelirsoft", "CATALOG_PROPS_AS_DROPDOWN"));
										// зачистим пробелы
							foreach ($tmp_dropdownSku as $arField) {
								$dropdownSku[] = trim($arField);
							}
							?>
							<?
							foreach ($arResult['SKU_PROPS'] as $skuProperty)
							{
								...
								$dropdown = false;
								if(in_array($skuProperty['CODE'], $dropdownSku)){
									$dropdown = true;
								}
								...
								<div class="product-item-detail-info-container" data-entity="sku-line-block">
									...
									<div class="product-item-scu-container">
										<div class="product-item-scu-block">
											<div class="<?=htmlspecialcharsEx($skuProperty['CODE']);?> product-item-scu-list<?=($dropdown ? " wrapper-dropdown" : "")?>">
												<?=($dropdown ? '<span class="ul_name"></span>' : "")?>
												<ul class="product-item-scu-item-list<?=($dropdown ? " dropdown" : "")?>">
													<?
												...
												<?if($skuProperty['CODE'] == 'RAZMER' && $arParams['USE_NO_SIZE'] == 'Y'){?>
						...
						if ($showProps)
						{
							...
							<div class="product-item-detail-info-container">
								...
								if (!empty($arResult['DISPLAY_PROPERTIES']))
								{
									...
									<div class="product-item-detail-properties">
										<?
										foreach ($arResult['DISPLAY_PROPERTIES'] as $propKey => $property)
										{
								...
								{
									?>
									<div class="product-item-detail-properties" id="<?=$itemIds['DISPLAY_MAIN_PROP_DIV']?>"></div>
									<?
								}
								?>
							</div>
						...
						if ($arParams['SHOW_MAX_QUANTITY'] !== 'N')
						{
							...
							{
								?>
								<div class="product-item-detail-info-container hidden" id="<?=$itemIds['QUANTITY_LIMIT']?>" style="display: none;">
									<div class="product-item-detail-info-container-title">
							...
							else
							{
								...
								{
									?>
									<div class="product-item-detail-info-container hidden" id="<?=$itemIds['QUANTITY_LIMIT']?>">
										<div class="product-item-detail-info-container-title">
											<?=$arParams['MESS_SHOW_MAX_QUANTITY']?>:
						...
						{
							?>
							<div class="product-item-detail-info-container hidden" style="<?=(!$actualItem['CAN_BUY'] ? 'display: none;' : '')?>"
						...
						case 'buttons':
						?>
						<div class="hidden" data-entity="main-button-container">
					...
					<div class="product-item-detail-info-container">
						...
						<p style="text-align: center;">
							...
							<span style="font-size: 12pt;">
								<b> Каталог интернет-магазина является ознакомительным,&nbsp;продажа происходит после фактического ознакомления покупателя с изделием при доставке курьером или самовывозе,&nbsp;оплачивается товар после осмотра.</b>
							</span>
						</p>
						*/?>
					</div>
				...
				<?/*
				<div class="product-item-detail-info-section">
					<div class="store-list-title">Наличие в магазинах <div class="btn btn-link" id="all_size_link">показать все</div></div>
					<div class="store-list-container" id="<?=$itemIds['STORE_LIST_ID']?>"></div>
				</div>
				*/?>
			</div>
		</div>
				<div class="product-item-detail-info-section store-list">
					<?/*<div style ="margin: 30px 0; font-size: 12pt; text-align: center;">
						<b> ВНИМАНИЕ! Сайт работает в тестовом режиме, точную информацию о наличии и стоимости изделия узнавайте по телефону магазина</b>
					</div>*/?>
					<div class="store-list-title"><span>Наличие в магазинах</span> <div class="btn btn-link" id="all_size_link">показать все</div></div>
					<div class="store-list-container" id="<?=$itemIds['STORE_LIST_ID']?>"></div>
				</div>
		<div class="row">
			<div class="col-xs-12">
...
<!--Top tabs-->
<div class="product-item-detail-tabs-container-fixed hidden-xs<?=(!$showDescription && !$showProps ? " hide" : "")?>" id="<?=$itemIds['TABS_PANEL_ID']?>">
	<ul class="product-item-detail-tabs-list">
		<?
		if ($showDescription)
		{
	...
	foreach ($arResult['JS_OFFERS'] as $ind => &$jsOffer)
	{
		...
		if ($arResult['SHOW_OFFERS_PROPS'])
		{
			if (!empty($jsOffer['DISPLAY_PROPERTIES']))
			{
				...
				{
					$current = '<div>'.'<span>'.$property['NAME'].':</span> <span>'.(
...
<script>
	...
	var obCatalogElementName = '<?=$obName?>';
</script>
...
<script type="text/javascript">
	...
	function toggleShopRow(athis,id) {
		$('.shop-list-offers').data('shop',id).each(function( index ) {
			if ($(athis).find('.fa').hasClass("fa-chevron-down") ){
		...
		if ($(athis).find('.fa').hasClass("fa-chevron-down") ){
			$(athis).find('.fa').removeClass('fa-chevron-down');
			$(athis).find('.fa').addClass('fa-chevron-up');
		} else {
			$(athis).find('.fa').removeClass('fa-chevron-up');
			$(athis).find('.fa').addClass('fa-chevron-down');
		}
	}
</script>
<?
