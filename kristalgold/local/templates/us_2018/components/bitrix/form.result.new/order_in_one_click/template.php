...
<?//if($FIELD_SID == 'PERSONAL') continue;?>
	...
	<?}?>
	<p>
		<input class='btn btn-subscribe' onclick="trySubmit_bok()" type="button" name="" value="<?=htmlspecialcharsbx(strlen(trim($arResult["arForm"]["BUTTON"])) <= 0 ? GetMessage("FORM_ADD") : $arResult["arForm"]["BUTTON"]);?>" />
		<input id='submitOrderInOneClick' class='btn btn-subscribe hide' <?=(intval($arResult["F_RIGHT"]) < 10 ? "disabled=\"disabled\"" : "");?> type="submit" name="web_form_submit" value="<?=htmlspecialcharsbx(strlen(trim($arResult["arForm"]["BUTTON"])) <= 0 ? GetMessage("FORM_ADD") : $arResult["arForm"]["BUTTON"]);?>" />
	</p>
<p>
