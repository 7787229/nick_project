...
if(!$USER->IsAuthorized()){
	...
	if(isset($_POST['send_account_info']) and isset($_POST['USER_LOGIN']) and !empty($_POST['USER_LOGIN'])){
		?>
			<h4>
			<?=GetMessage("At the specified e-mail address sent instructions to reset your password.")?>
			</h4>	
			<script>
				$(document).ready(function(){
					$("#auth_and_register_tabs").modal("show");
	...
	?>
		<div class="bx-authform">
				<h4><?=GetMessage("You are already logged in and password recovery is not needed! Use the form change the registration data.")?></h4>
		</div>
	<?
