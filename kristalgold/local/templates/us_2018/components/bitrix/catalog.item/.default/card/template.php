...
<div class="product-item" property="itemListElement" typeof="ListItem">
<div property="item" typeof="Product">
	<link href="<?=$item['DETAIL_PAGE_URL']?>" property="url">
	<meta property="name" content="<?=$item['NAME']?>">
	...
	<a class="product-item-image-wrapper" href="<?=$item['DETAIL_PAGE_URL']?>" title="<?=$imgTitle?>"
		...
		$discountPersent = false;

		if ($arParams['SHOW_DISCOUNT_PERCENT'] === 'Y')
		{
			if(array_key_exists('DISCOUNT', $item['LABEL_ARRAY_VALUE']) && $price['PERCENT'] > 0){
				$discountPersent = "-".$price['PERCENT']."%";
			}else{
				?>
					...
					foreach ($item['LABEL_ARRAY_VALUE'] as $code => $value)
					{
						...
						switch ($code) {
							...
							case 'BESTSELLER':
								...
								break;
							case 'DISCOUNT':
								$value = $discountPersent === false ? GetMessage("DISCOUNT") : $discountPersent;
								$class = " item_".strtolower($code);
								break;
							default: break;
						}
						?>
						<div<?=(!isset($item['LABEL_PROP_MOBILE'][$code]) ? ' class="hidden-xs'.$class.'"' : $class)?>>
	...
	<div class="product-item-title">
		<a href="<?=$item['DETAIL_PAGE_URL']?>" title="<?=$productTitle?>" target='_self'><?=$productTitle?></a>
	</div>
	<?
	if (!empty($arParams['PRODUCT_BLOCKS_ORDER']))
	{
		...
		foreach ($arParams['PRODUCT_BLOCKS_ORDER'] as $blockName)
		{
			...
			{
				case 'price': ?>
					<div class="product-item-info-container product-item-price-container" data-entity="price-block" property="offers" typeof="Offer">
						<meta property="priceCurrency" content="RUB">
						<meta property="name" content="<?echo $productTitle;?>">
						...
						if ($arParams['ACTIONS'] && !empty($item["PROPERTIES"]["MIN_ACTION_NAME"]["VALUE"]) )
						{
							?>
							<span class="product-item-price-old" id="<?=$itemIds['PRICE_OLD']?>">
								<?=CurrencyFormat($item["PROPERTIES"]["MIN_ACTION_OLD_PRICE"]["VALUE"],'RUB')?>
							</span>
							<br>
							<span property="price" class="product-item-price-current product-item-price-current-red" id="<?=$itemIds['PRICE']?>">
								<?=CurrencyFormat($item["PROPERTIES"]["MIN_ACTION_PRICE"]["VALUE"],'RUB')?>
							</span>
							<?
						} else {
						?>
						<span property="price" class="product-item-price-current" id="<?=$itemIds['PRICE']?>">
						<?
						if (!empty($price))
						{
							/*
				...
				case 'quantity':
					if (!$haveOffers && $arParams['PRODUCT_DISPLAY_MODE'] === 'Y')
					{
				...
				case 'props':
					if (!$haveOffers)
					{
						if (!empty($item['DISPLAY_PROPERTIES']) && $arResult['SCALABLE'] === 'Y')
						{
							?>
					...
					else
					{
						...
						if (($showProductProps || $showOfferProps) && $arResult['SCALABLE'] === 'Y')
						{
							?>
							<div class="product-item-info-container product-item-hidden" data-entity="props-block">
								<div class="product-item-properties">
				...
				case "rating":
					if (
						...
						&& ($arResult["SCALABLE"] == 'Y' || $arParams['PRODUCT_DISPLAY_MODE'] == 'Y')
					)
					{
						?>
