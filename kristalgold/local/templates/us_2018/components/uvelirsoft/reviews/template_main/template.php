	...
	<?if($ajax == 'Y'){
		...
		<div id='user_reviwe_or_comment_block'>
			...
			foreach ($arResult["ITEMS"] as $arItem) {
				...
				if(htmlspecialcharsback($arItem["PREVIEW_TEXT"]))
				?>
				<div class='row review-block col-md-12<?=($i > $arParams["COUNT_PAGE"] && $_POST['show_review_items'] != 'Y' ? " hide" : "")?>' id='<?=$this->GetEditAreaId($arItem['ID']);?>'>
					<div class="col-md-12 review-personal-info">

						<div class="review-autor">
							<?=$arItem["PROPS"]["USER_NAME"]["VALUE"] ? $arItem["PROPS"]["USER_NAME"]["VALUE"] : $arItem['NAME']?>	
						</div>
					...
					<div class='col-md-12 review-item' id='<?=$arParams["IBLOCK_ID"]?>_review_<?=$arItem["ID"]?>'>
						<?if(htmlspecialcharsback($arItem["PROPS"]["REVIEW_TITLE"]["VALUE"])){?>
							<div class='review-item-title'>
								<?=htmlspecialcharsback($arItem["PROPS"]["REVIEW_TITLE"]["VALUE"])?>
							</div>
						<?}?>
						...
						foreach ($arItem["REPLAYS"] as $arItemCH) {
							...
							if(empty(htmlspecialcharsback($arItemCH["PREVIEW_TEXT"]))) continue;
							?>
							<div class='row review-block' id='<?=$this->GetEditAreaId($arItemCH['ID']);?>'>
								<div id='<?=$arParams["IBLOCK_ID"]?>_review_<?=$arItemCH["ID"]?>' class="col-md-12">
									<div class="review-autor">
										<?=$arItemCH["PROPS"]["USER_NAME"]["VALUE"] ? $arItemCH["PROPS"]["USER_NAME"]["VALUE"] : $arItemCH['NAME']?>	
									</div>
									...
									?>
									<div class="review-status"><?=implode(" | ",$arStatus)?></div>
									<?if(htmlspecialcharsback($arItemCH["PROPS"]["REVIEW_TITLE"]["VALUE"])){?>
										<div class='review-item-title'>
											<?=htmlspecialcharsback($arItemCH["PROPS"]["REVIEW_TITLE"]["VALUE"])?>
										</div>
									<?}?>
									<div class='review-item-text'>
										<?=htmlspecialcharsback($arItemCH["PREVIEW_TEXT"])?>
									</div>
			...
			function showForm(form_container, parent, title){
				$.post(
					"<?=$templateFolder?>/form.php",
					{
						...
						reviewname: '<?=(htmlspecialcharsback($_POST["reviewname"]) ? htmlspecialcharsback($_POST["reviewname"]):(htmlspecialcharsback(trim($USER->GetFirstName()." ".$USER->GetLastName()))))?>',
						reviewmail: '<?=(htmlspecialcharsback($_POST["reviewmail"]) ? htmlspecialcharsback($_POST["reviewmail"]):(htmlspecialcharsback(trim($USER->GetEmail()))))?>',
						reviewphone: '<?=(htmlspecialcharsback($_POST["reviewphone"]) ? htmlspecialcharsback($_POST["reviewphone"]): ($arUser['PERSONAL_MOBILE']?$arUser['PERSONAL_MOBILE']:$arUser['PERSONAL_PHONE']))?>',
						reviewcity: '<?=(htmlspecialcharsback($_POST["reviewcity"]) ? htmlspecialcharsback($_POST["reviewcity"]): $arUser['PERSONAL_CITY'])?>',
						reviewtext: '<?=(htmlspecialcharsback($_POST["reviewtext"]) ? htmlspecialcharsback($_POST["reviewtext"]):"")?>',
