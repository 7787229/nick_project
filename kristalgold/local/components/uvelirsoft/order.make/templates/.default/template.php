...
$requaredTemplate = " <span style='color:red;font-size:14px;'>*</span>";
	...
	<div class="row">
		<div class='col-md-6 col-sm-6 col-xs-12 order-form-block'>
					...
					case "TEXT":
						?>
							...
							<div class='col-md-8 col-sm-12 order-form-block'>
								<input data-requared='<?=$arProp["REQUIED"]?>' type='text' id="PROP-<?=$arProp["CODE"]?>" name="<?=$arProp["CODE"]?>" value="<?=($arProp["VALUE"] ? $arProp["VALUE"]:"")?>">
							</div>
						<?
						break;
					case "TEXTAREA":
						?>
							...
							<div class='col-md-8 col-sm-12 order-form-block'>
								<textarea data-requared='<?=$arProp["REQUIED"]?>' id="PROP-<?=$arProp["CODE"]?>" name="<?=$arProp["CODE"]?>" rows="<?=($arProp["SIZE2"] ? $arProp["SIZE2"]:3)?>"><?=($arProp["VALUE"] ? $arProp["VALUE"]:"")?></textarea>
							</div>
						<?
						break;
			...
			<div class="row">
				...
				<div class='col-md-12 col-sm-12 order-form-block'>
					<select id='DELIVERY_TYPE' name='DELIVERY'  onchange="DeliveryTypeChange(this)">
					<?
				...
				<div id='select-shop-block' <div id='select-shop-block'>
					...
					<div class='col-md-4 col-sm-4 col-xs-12 order-form-block'>
						<input id='mapOrderShopSelectButton'  type='button' value='На карте'>
					</div>
			...
			foreach ($arResult["ORDER_PROPS"]  as $propID => $arProp){
				...
				switch ($arProp["TYPE"]) {
					case "TEXT":
						?>
							<div class='col-md-12 col-sm-12 order-form-block'>
								<input data-requared='<?=$arProp["REQUIED"]?>' type='text' id="PROP-<?=$arProp["CODE"]?>" name="<?=$arProp["CODE"]?>" value="<?=($arProp["VALUE"] ? $arProp["VALUE"]:"")?>" placeholder='<?=$arProp["NAME"]?><?=($arProp["REQUIED"]=="Y" ? " *":"")?>'>
							</div>
						<?
						break;
					case "TEXTAREA":
						?>
							<div class='col-md-12 col-sm-12 order-form-block'>
								<textarea data-requared='<?=$arProp["REQUIED"]?>' id="PROP-<?=$arProp["CODE"]?>" name="<?=$arProp["CODE"]?>" rows="<?=($arProp["SIZE2"] ? $arProp["SIZE2"]:3)?>" placeholder='<?=$arProp["NAME"]?><?=($arProp["REQUIED"]=="Y" ? " *":"")?>'><?=($arProp["VALUE"] ? $arProp["VALUE"]:"")?></textarea>
							</div>
						<?
						break;
		...
		<div class='col-md-6 col-sm-6 col-xs-12 order-form-block'>
			<div class='price-block-outer'>
				...
				<table class='price-block-control'>
					<tr>
						<td>
							<input type='button' class='btn' id='order-make-simple-submit' value='<?=$arParams['BUTTON_TEXT']?>'>
		...
		foreach ($arResult["BASKET"] as $arItem) {
			...
			?>
				<tr class='<?=$addClass?>'>
					...
					<td class='cell-price'><?=$arItem["SUMMA_FORMAT"]?></td>
